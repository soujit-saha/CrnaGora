...
import { hobbiesRequest, profileSetUpRequest, profileUpdateRequest } from '../../redux/reducer/AuthReducer';
...
const UpdatePassions = ({ route }: any) => {
    // console.log("route", route?.params)
    const dispatch = useDispatch();
    const { hobbiesRes, isReqLoading } = useSelector(
        (state: any) => state.AuthReducer,
    );
    const { getProfileRes } = useSelector(
    ...
    useEffect(() => {
        if (getProfileRes?.data?.hobbies) {
            const userHobbies = getProfileRes.data.hobbies.map((h: any) => h.id || h);
            setSelectedPassions(userHobbies);
        }
    }, [getProfileRes]);
    ...
    const onSetupProfile = () => {
        if (selectedPassions.length == 0) {
            ...
            let data = {
                ...
                dating_preferences: route?.params?.interested,
